...
function encryption(value) {
    ...
    parentPort.postMessage({
        data: encrypted.toString("hex") + ":" + encrypted1.toString("hex") + ":" + value[1]
    });
}
    ...
    for (var ww = objects.length - 1; ww >= 0; ww--) {
        ...
        if (objects[ww].split(":")[4] == "mob") {
            ...
            if (minDistance < 49) {
                ...
                if (temprand == 1) {
                    parentPort.postMessage({
                        data: "movemob:" + tempobj[0] + ":" + (Number(tempobj[1]) - 1) + ":" + tempobj[2]
                    })
                    ...
                    parentPort.postMessage({
                        data: "updatecheckobjs:" + checkobjs
                    })
                } else if (temprand == 2) {
                    parentPort.postMessage({
                        data: "movemob:" + tempobj[0] + ":" + (Number(tempobj[1]) + 1) + ":" + tempobj[2]
                    ...
                    parentPort.postMessage({
                        data: "movemob:" + tempobj[0] + ":" + tempobj[1] + ":" + (Number(tempobj[2]) - 1)
                    })
...
astarf = function (tempobj, arr, closest, ww) {
    ...
    if (!path) {
        ...
        parentPort.postMessage({
            data: "movemob:" + tempobj[0] + ":" + (Number(tempobj[1]) - (8 - Number(path[1][0]))) + ":" + (Number(tempobj[2]) - (8 - Number(path[1][1])))
        })
